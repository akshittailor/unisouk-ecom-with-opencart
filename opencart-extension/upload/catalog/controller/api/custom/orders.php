<?php
namespace Opencart\Catalog\Controller\Api\Custom;
...
class Orders extends \Opencart\System\Engine\Controller {
    ...
    private function isAuthorized(): bool {
        $api_token = (string)($this->request->get['api_token'] ?? '');
        $header_token = (string)($this->request->server['HTTP_X_OPENCART_API_TOKEN'] ?? '');
        $bearer = (string)($this->request->server['HTTP_AUTHORIZATION'] ?? '');
        ...
        if ($bridge_secret_config !== '' && $bridge_secret_header !== $bridge_secret_config) {
            return false;
        }

        if ($api_token !== '' || $header_token !== '') {
            return true;
        }

        return stripos($bearer, 'Bearer ') === 0;
    }
}
